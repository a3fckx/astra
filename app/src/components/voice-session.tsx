...
export function VoiceSession({ agentId }: VoiceSessionProps) {
  ...
  useEffect(() => {
     ...
     * Once connected, send a contextual update to ElevenLabs with the session metadata.
     * This allows the agent to reference user identity, workflow ID, and token availability
     * during the conversation. We only send this once per connection.
     */
    ...
    console.info(
      "[Contextual Update] Preparing to send session context to ElevenLabs",
      {
        ...
        memoryStoreTokenAvailable: contextPayload.memoryStoreTokenAvailable,
      },
    );
