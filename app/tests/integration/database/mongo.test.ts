...
describe("MongoDB Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    client = (await import("../../helpers/mongodb")).getTestMongoClient();
  });
  ...
  describe("Collection Operations", () => {
    ...
    describe("Sessions Collection", () => {
      it("should create and retrieve a session", async () => {
        ...
        // Retrieve session
        const retrievedSession = await sessionsCollection.findOne({
          user_id: testSession.user_id,
        });
        expect(retrievedSession).toBeDefined();
        expect(retrievedSession?.julep_session_id).toBe(
          testSession.julep_session_id,
        );
    ...
    describe("ElevenLabs Conversations Collection", () => {
      it("should create and retrieve conversations", async () => {
        ...
        const retrievedConversation = await conversationsCollection.findOne({
          user_id: "user_123",
        });
        expect(retrievedConversation).toBeDefined();
        expect(retrievedConversation?.conversation_id).toBe(
      ...
      it("should handle different workflow types", async () => {
        ...
        const horoscopeConversations = await conversationsCollection
          .find({ user_id: userId, workflow_id: "horoscope-workflow" })
          .toArray();
        expect(horoscopeConversations).toHaveLength(1);
        expect(horoscopeConversations[0].workflow_id).toBe(
          "horoscope-workflow",
        );
      });
    });
    ...
    describe("Integration Tokens Collection", () => {
      it("should create and retrieve integration tokens", async () => {
        ...
        // Retrieve token
        const retrievedToken = await tokensCollection.findOne({
          userId: "user_123",
        });
      ...
      it("should find tokens by integration type", async () => {
        ...
        const memoryToken = createTestIntegrationToken({
          userId,
          integration: "memory-store",
        });
        const elevenlabsToken = createTestIntegrationToken({
          userId,
          integration: "elevenlabs",
        });
        const anotherMemoryToken = createTestIntegrationToken({
          userId,
          integration: "memory-store",
        });

        await tokensCollection.insertMany([
          memoryToken,
          elevenlabsToken,
          anotherMemoryToken,
        ]);
        ...
        const memoryTokens = await tokensCollection
          .find({ userId, integration: "memory-store" })
          .toArray();
        expect(memoryTokens).toHaveLength(2);
        expect(
          memoryTokens.every((t) => t.integration === "memory-store"),
        ).toBe(true);

        // Find elevenlabs tokens
        const elevenlabsTokens = await tokensCollection
          .find({ userId, integration: "elevenlabs" })
          .toArray();
  ...
  describe("Data Integrity and Validation", () => {
    ...
    it("should handle complex metadata objects", async () => {
      ...
      const insertResult = await tokensCollection.insertOne(
        tokenWithComplexMetadata,
      );
      const retrievedToken = await tokensCollection.findOne({
        _id: insertResult.insertedId,
      });
      ...
      expect(retrievedToken?.metadata?.tags).toEqual([
        "urgent",
        "support",
        "billing",
      ]);
